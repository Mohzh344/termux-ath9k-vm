# termux-ath9k-vm — Lite v0.3.4

A lightweight Alpine Linux ARM64 VM companion for Android/Termux and QEMU TCG without KVM. Lite v0.3.4 is an additive corrected image. The original Full v0.3.0 tag remains available unchanged; the unified v0.3.4 archive ships a documented console-shell/PATH patch for its bundled Full guest.
...
The default Lite console is a private local QEMU serial root shell started directly by init as `/bin/sh -l`. The `-l` flag only makes BusyBox `ash` read `/etc/profile` and `/etc/profile.d`; it is not a username/password login program. The unified launcher also offers `login` mode for a normal BusyBox username/password prompt and `login-empty` for private local testing with an intentionally empty root password. The image explicitly keeps `/usr/local/bin` in `PATH`, so commands installed there are available immediately. Do not expose the serial socket or enable remote login without adding a deliberate authentication policy.

## Console authentication modes

The recommended mode is the default direct root console:

```sh
AUTH_MODE=root-console bash ../bin/vm-launcher.sh --lite
```

For a normal username/password prompt using the current root password:

```sh
AUTH_MODE=login bash ../bin/vm-launcher.sh --lite
```

For local testing only, with an empty root password:

```sh
AUTH_MODE=login-empty bash ../bin/vm-launcher.sh --lite
```

The image must be stopped before the launcher changes `/etc/inittab`, `/etc/passwd`, or `/etc/shadow`. Password changes now persist because Full boots with `rootflags=rw`; Lite already mounts its ext4 image writable.
...
See `docs/LITE-v0.3.4.md` for the correction details and reproducible rebuild command.
