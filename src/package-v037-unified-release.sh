...
OUT="${OUT:-$DIST_DIR/termux-ath9k-vm-v037-full-lite-ready.tar.gz}"
STAGE="$(mktemp -d "${TMPDIR:-/tmp}/termux-ath9k-vm-v037-unified.XXXXXX")"
...
# Include the general bootstrap installer and the bundled in-place updater.
cp -p "$BASE_DIR/install.sh" "$ROOT/install.sh"
cp -p "$BASE_DIR/bin/awvm-update.sh" "$ROOT/bin/awvm-update.sh"

# Use the corrected Lite scripts, patched guest consoles, and v0.3.7 documentation.
rm -f "$ROOT/docs/LITE-v0.3.1.md" "$ROOT/docs/RELEASE-v0.3.1.md" "$ROOT/docs/LITE-v0.3.3.md" "$ROOT/docs/RELEASE-v0.3.3.md" "$ROOT/docs/LITE-v0.3.4.md" "$ROOT/docs/RELEASE-v0.3.4.md" "$ROOT/docs/LITE-v0.3.5.md" "$ROOT/docs/RELEASE-v0.3.5.md" "$ROOT/docs/LITE-v0.3.6.md" "$ROOT/docs/RELEASE-v0.3.6.md" "$ROOT/docs/LITE-v0.3.7.md" "$ROOT/docs/RELEASE-v0.3.7.md"
cp -p "$BASE_DIR/docs/LITE-v0.3.7.md" "$ROOT/lite/README.md"
...
cp -p "$BASE_DIR/docs/LITE-v0.3.7.md" "$BASE_DIR/docs/RELEASE-v0.3.7.md" "$BASE_DIR/docs/UNIFIED-RELEASE.md" "$ROOT/docs/"
chmod +x "$ROOT/install.sh" "$ROOT/bin/"*.sh "$ROOT/src/"*.sh "$ROOT/lite/bin/"*.sh "$ROOT/lite/src/"*.sh "$ROOT/full/src/"*.sh
...
(
  cd "$ROOT/lite/guest"
  sha256sum alpine-ath9k-v030-lite.img vmlinuz-tiny vmlinuz-safe vmlinuz-lts-lite initramfs-lts-lite > SHA256SUMS-v0.3.7-lite
)
...
if [ "$INCLUDE_IMAGES" = 0 ]; then
  ...
  (
    ...
    sha256sum vmlinuz-tiny vmlinuz-safe vmlinuz-lts-lite initramfs-lts-lite > SHA256SUMS-v0.3.7-update
  )
