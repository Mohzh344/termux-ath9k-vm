...
OUT="${OUT:-$DIST_DIR/termux-ath9k-vm-v032-full-lite-ready.tar.gz}"
STAGE="$(mktemp -d "${TMPDIR:-/tmp}/termux-ath9k-v032-unified.XXXXXX")"
...
# Use the corrected Lite scripts and the new unified adapter/documentation. Full nested files are untouched.
rm -f "$ROOT/docs/LITE-v0.3.1.md" "$ROOT/docs/RELEASE-v0.3.1.md" "$ROOT/docs/LITE-v0.3.2.md" "$ROOT/docs/RELEASE-v0.3.2.md"
cp -p "$BASE_DIR/README-lite-v0.3.2.md" "$ROOT/lite/README.md"
...
cp -p "$BASE_DIR/docs/LITE-v0.3.2.md" "$BASE_DIR/docs/RELEASE-v0.3.2.md" "$BASE_DIR/docs/UNIFIED-RELEASE.md" "$ROOT/docs/"
...
(
  cd "$ROOT/lite/guest"
  sha256sum alpine-ath9k-v030-lite.img vmlinuz-tiny vmlinuz-safe vmlinuz-lts-lite initramfs-lts-lite > SHA256SUMS-v0.3.2-lite
)
