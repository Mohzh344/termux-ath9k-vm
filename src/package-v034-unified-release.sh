...
OUT="${OUT:-$DIST_DIR/termux-ath9k-vm-v034-full-lite-ready.tar.gz}"
STAGE="$(mktemp -d "${TMPDIR:-/tmp}/termux-ath9k-v034-unified.XXXXXX")"
...
for f in \
  ...
  [ -s "$f" ] || die "required Lite artifact not found for v0.3.4: $f"
done
mkdir -p "$DIST_DIR"
...
"$BASE_DIR/src/configure-console-auth.sh" "$ROOT/full/guest/alpine-ath9k.img" root-console
...
"$BASE_DIR/src/configure-console-auth.sh" "$ROOT/lite/guest/alpine-ath9k-v030-lite.img" root-console
...
# Use the corrected Lite scripts, patched guest consoles, and v0.3.4 documentation.
rm -f "$ROOT/docs/LITE-v0.3.1.md" "$ROOT/docs/RELEASE-v0.3.1.md" "$ROOT/docs/LITE-v0.3.3.md" "$ROOT/docs/RELEASE-v0.3.3.md" "$ROOT/docs/LITE-v0.3.4.md" "$ROOT/docs/RELEASE-v0.3.4.md"
cp -p "$BASE_DIR/README-lite-v0.3.4.md" "$ROOT/lite/README.md"
...
mkdir -p "$ROOT/src"
cp -p "$BASE_DIR/src/configure-console-auth.sh" "$BASE_DIR/src/patch-full-builder-shadow.py" "$ROOT/src/"
...
cp -p "$BASE_DIR/src/build-lite-image.sh" "$BASE_DIR/src/guest-install-wifi-tools.sh" "$BASE_DIR/src/package-lite-release.sh" "$BASE_DIR/src/build-kernel-tiers.sh" "$BASE_DIR/src/verify-kernel-tiers.sh" "$BASE_DIR/src/patch-console-login-shell.sh" "$BASE_DIR/src/configure-console-auth.sh" "$ROOT/lite/src/"
cp -p "$BASE_DIR/docs/LITE-v0.3.4.md" "$BASE_DIR/docs/RELEASE-v0.3.4.md" "$BASE_DIR/docs/UNIFIED-RELEASE.md" "$ROOT/docs/"
chmod +x "$ROOT/bin/"*.sh "$ROOT/src/"*.sh "$ROOT/lite/bin/"*.sh "$ROOT/lite/src/"*.sh
...
(
  cd "$ROOT/lite/guest"
  sha256sum alpine-ath9k-v030-lite.img vmlinuz-tiny vmlinuz-safe vmlinuz-lts-lite initramfs-lts-lite > SHA256SUMS-v0.3.4-lite
)
