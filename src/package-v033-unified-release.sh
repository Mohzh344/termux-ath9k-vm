...
OUT="${OUT:-$DIST_DIR/termux-ath9k-vm-v033-full-lite-ready.tar.gz}"
STAGE="$(mktemp -d "${TMPDIR:-/tmp}/termux-ath9k-v033-unified.XXXXXX")"
...
for f in \
  ...
  [ -s "$f" ] || die "required Lite artifact not found for v0.3.3: $f"
done
mkdir -p "$DIST_DIR"
...
# Use the corrected Lite scripts, patched guest consoles, and v0.3.3 documentation.
rm -f "$ROOT/docs/LITE-v0.3.1.md" "$ROOT/docs/RELEASE-v0.3.1.md" "$ROOT/docs/LITE-v0.3.3.md" "$ROOT/docs/RELEASE-v0.3.3.md"
cp -p "$BASE_DIR/README-lite-v0.3.3.md" "$ROOT/lite/README.md"
...
cp -p "$BASE_DIR/src/build-lite-image.sh" "$BASE_DIR/src/guest-install-wifi-tools.sh" "$BASE_DIR/src/package-lite-release.sh" "$BASE_DIR/src/build-kernel-tiers.sh" "$BASE_DIR/src/verify-kernel-tiers.sh" "$BASE_DIR/src/patch-console-login-shell.sh" "$ROOT/lite/src/"
cp -p "$BASE_DIR/docs/LITE-v0.3.3.md" "$BASE_DIR/docs/RELEASE-v0.3.3.md" "$BASE_DIR/docs/UNIFIED-RELEASE.md" "$ROOT/docs/"
...
(
  cd "$ROOT/lite/guest"
  sha256sum alpine-ath9k-v030-lite.img vmlinuz-tiny vmlinuz-safe vmlinuz-lts-lite initramfs-lts-lite > SHA256SUMS-v0.3.3-lite
)
