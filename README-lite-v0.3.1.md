# termux-ath9k-vm — Lite v0.3.1

A lightweight Alpine Linux ARM64 VM companion for Android/Termux and QEMU TCG without KVM. Lite v0.3.1 is an additive corrected image; the canonical Full v0.3.0 release remains unchanged and is shipped beside it in the unified archive.
...
Lite stays offline in the default `wifi-only` profile. Enable QEMU user networking explicitly when package installation is needed:

```sh
ENABLE_NET=1 PROFILE=balanced KERNEL_TIER=safe ./bin/launch-vm-lite.sh
```

When a virtio NIC is present, the guest automatically brings up `eth0`, obtains a DHCP lease, and uses the QEMU DNS proxy at `10.0.2.3`. If no network interface is available, `install-wifi-tools` stops with an actionable message rather than attempting a guaranteed-to-fail apk transaction.
...
See `docs/LITE-v0.3.1.md` for the correction details and reproducible rebuild command.
