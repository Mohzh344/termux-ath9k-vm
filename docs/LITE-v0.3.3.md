...
Lite v0.3.3 is a small Alpine ARM64 guest for QEMU TCG on Android/Termux without Android root. It is optimized for an external AR9271/ath9k_htc adapter and keeps the console local and passwordless. The ttyAMA0 action starts `/bin/sh -l` directly as root; this is a login shell for profile loading, not a username/password login program. The original Full v0.3.0 tag remains unchanged; the unified v0.3.3 archive applies the same documented console-shell/PATH patch to its bundled Full guest.
...
The unified launcher reads the Android/Termux host clock with `date -u` on every launch, passes it to QEMU as `-rtc base=...`, waits for the local root console, and executes `date -u -s` inside Alpine before handing the console to the user. The guest console is configured as `ttyAMA0::respawn:/bin/sh -l`, so BusyBox `ash` reads `/etc/profile` and `/etc/profile.d`; `/etc/profile.d/wifi-vm.sh` also explicitly exports the standard `/usr/local/bin` path. This explicit guest-side step is required because the minimal ARM64 kernels do not expose a usable `/dev/rtc` device to userspace. The image builder installs and refreshes the CA bundle after no-script provisioning and places the server trust bundle at `/etc/apk/ca.pem`, the path used by libapk. The installer tries strict certificate verification first. If the guest reports a certificate failure, it can retry with `--check-certificate=no` when `APK_ALLOW_INSECURE_FALLBACK` is not set to `0`; package signatures remain verified by apk. Set that variable to `0` for strict TLS-only behavior.
...
The guest uses a private local serial root console with no default password. The `-l` argument only loads shell startup files; it does not invoke `login`, ask for a username, or validate a password. This is intentional for the local QEMU console and is not an SSH or remote-login policy. Do not expose the serial socket or add network login without creating an explicit authentication policy.
