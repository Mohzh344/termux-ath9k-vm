...
The unified archive is an additive distribution layer. It contains the published Full v0.3.0 bundle under `full/` unchanged and the corrected Lite v0.3.2 bundle under `lite/`. The top-level `bin/vm-launcher.sh` detects complete bundles, asks whether Internet should be granted, seeds the Android clock, and delegates to the appropriate launch path.
...
The existing Full release remains available at the original `v0.3.0` tag and release. The combined release does not replace that tag. Its nested `full/` directory is a copy of the published Full archive, while its nested `lite/` directory contains the rebuilt v0.3.2 Lite image, file-locking custom kernels, automatic DHCP setup, corrected apk installer, and default time synchronization. The new top-level dispatcher and `bin/launch-vm-full-unified.sh` adapter remain the convenience layer added by this release; nested Full files are untouched.
