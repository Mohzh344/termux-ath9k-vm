...
This archive contains both the canonical **v0.3.0 Full** VM and the additive **v0.3.0 Lite** VM in one download. You can keep both variants installed and select the one that fits the current task.
...
| **Lite / safe** | Recommended everyday AR9271/ath9k_htc use on Android with a compatibility-oriented custom kernel | Direct-root `vmlinuz-safe` |
| **Lite / tiny** | AR9271-only use when the smallest custom kernel is preferred | Direct-root `vmlinuz-tiny` |
...
Tier A and Tier B are kernel builds, not Wi-Fi monitor-mode switches. Both Lite custom tiers contain the required ath9k_htc, cfg80211, mac80211, XHCI, virtio, and ext4 support. Wi-Fi operations remain inside the guest and should only be performed on networks and devices you own or are authorized to test.
...
├── full/                           # canonical Full v0.3.0 bundle
│   ├── bin/
│   ├── guest/alpine-ath9k.img
│   └── guest/vmlinuz-lts + initramfs-lts
├── lite/                           # additive Lite v0.3.0 bundle
│   ├── bin/
